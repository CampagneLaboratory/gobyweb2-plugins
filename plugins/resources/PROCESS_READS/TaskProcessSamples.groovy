...
public class TaskProcessSample {
    ...
    private void calculateHeptamersWeights(final File sample) {
        ...
        try {
            ...
            if (sample && sample.exists() && heptamerInfoFile.exists()) {
                ...
                String cl = "${RESOURCES_ARTIFACTS_JAVA_LINUX_BINARIES}/bin/java ${jvmFlags} -jar ${gobyJarDir}/goby3.jar -m reads-to-weights ${sample} ${colorSpaceOpt} --method heptamers --heptamer-info ${heptamerInfoFile}"
    ...
    private void calculateGCWeights(final File sample) {
        ...
        try {
            if (sample && sample.exists()) {
                ...
                String cl = "${RESOURCES_ARTIFACTS_JAVA_LINUX_BINARIES}/bin/java ${jvmFlags} -jar ${gobyJarDir}/goby3.jar -m reads-to-weights ${sample} ${colorSpaceOpt} --method gc "
    ...
    private void getReadQualityStats(final File sample) {
        ...
        if (sample && sample.exists()) {
            String cl = "${RESOURCES_ARTIFACTS_JAVA_LINUX_BINARIES}/bin/java ${jvmFlags} -jar ${gobyJarDir}/goby3.jar -m read-quality-stats -o ${readQualChartFilename} ${sample} "
