...
public class TaskProcessSample {
    ...
    private int processSingleSample() {
        ...
        pairedSamplesFound = removePairs(detailsMap)
        sampleName = (detailsMap.keySet() as List)[0]
        filenameToSampleDetailsMap = sampleDetailsMapToFilenameMap(detailsMap)
        println "filenameToSampleDetailsMap= ${filenameToSampleDetailsMap}"
        println "sample name = ${sampleName}"
        ...
        if (pairedSamplesFound) {
            ...
            println "processFilePaths: ${noTagToProcessMap}"
            ...
            println "processFilePaths: ${processFilePaths}"
            renamed = renamePairFiles(filenameToSampleDetailsMap, processFilePaths, noTagToProcessMap)
        } else {
        ...
        String pairFilename
        File tmp_source = new File("./tmp-source");
        for (String processFilePath in processFilePaths) {
            println "Processing file ${processFilePath}"
            //the file was renamed because it was paired end
            if (renamed.contains(processFilePath)) {
                processFilePath = tmp_source.getAbsolutePath() + File.separator + new File(processFilePath).getName()
            }
            pairFilename = null
                ...
                try {
                    ...
                    if (processFqGzTar) {
                        ...
                        if (pairedSamplesFound) {
                            ...
                            final String originalFilenameLc = new File(processFilePath).getName();
                            println "Search pair info for ${originalFilenameLc}"
                            println "Search pair info in ${filenameToSampleDetailsMap}"
    ...
    private List<String> renamePairFiles(final Map<String, SampleDetails> filenameToSampleDetailsMap,
            ...
            if (sampleDetails) {
                ...
                if (pairKey) {
                    ...
                    if (originalPairFilename) {
                        ...
                        final File renameFrom = new File("${originalPairFilename}")
                        final File renameTo = new File("$newPairFilename")
                        println "Renaming pair from ${renameFrom} to ${renameTo} (first file in pair is ${processFilename})"
                        if (! new File(tmp_source, new File(processFilename).getName()).exists())
                            FileUtils.copyFileToDirectory(new File(processFilename), tmp_source)
                        if (! new File(tmp_source, renameTo.getName()).exists()) {
                            FileUtils.copyFileToDirectory(renameFrom, tmp_source)
                            File toRename = new File(tmp_source, renameFrom.getName());
                            toRename.renameTo(new File(tmp_source, renameTo.getName()));
                        }
                        renamedProcessFilenames.add(processFilename)
                    } else {
        ...
        return renamedProcessFilenames;
    }
    ...
    public makePairFilename(final String firstFilename, final List<String> pairIndicators) {
        ...
        String filenameNoExt = firstFilename; //FilenameUtils.removeExtension(firstFilename)
    ...
    /** Given a sampleDetailsMap, create a map of lowercase filename to SampleDetails.
     * @param sampleDetailsMap the sample details map
     * @return map of filename to SampleDetails
     */
    public Map<String, SampleDetails> sampleDetailsMapToFilenameMap(
            final Map<String, List<SampleDetails>> sampleDetailsMap) {
        final Map<String, SampleDetails> result = new HashMap<String, SampleDetails>()
        sampleDetailsMap.each { String sampleName, List<SampleDetails> sampleDetailsList ->
            sampleDetailsList.each { SampleDetails sampleDetails ->
                result[sampleDetails.filename] = sampleDetails
            }
        }
        return result
    }
    ...
    /**
     * When converting FA->CR, the fasta-to-compact mode takes only the first side of the pair,
     * automatically finds the second part of the pair. This will remove the second part of the
     * pair and return true if any second parts where removed.
     *
     * This is for use in TaskProcessSample.groovy. In this code, the map will be ONLY for a single
     * sample, if your Map has more than one key (sample name) you probably don't want to
     * be using this method.
     *
     * @param detailsMap the sample details map
     * @return true if pairs were found
     */
    public static boolean removePairs(final Map<String, List<SampleDetails>> detailsMap) {
        def removeFromList = []
        def removedPairs = false
        detailsMap.each { String sampleName, List<SampleDetails> sampleDetailsList ->
            removeFromList.clear()
            for (final SampleDetails sampleDetails in sampleDetailsList) {
                if (sampleDetails.pairing != 0) {
                    removeFromList << sampleDetails
                    removedPairs = true
                }
            }
            removeFromList.each { SampleDetails sampleDetails ->
                sampleDetailsList.remove(sampleDetails)
            }
        }
        return removedPairs
    }
