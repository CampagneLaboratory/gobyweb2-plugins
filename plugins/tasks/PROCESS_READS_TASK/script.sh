...
function plugin_task {
    ...
    until [ ${RETURN_STATUS} -eq 0 ] || [ ${CURRENT_RETRY} -gt ${MAX_RETRIES} ]; do
        ...
        ${RESOURCES_GROOVY_EXECUTABLE} -cp ${RESOURCES_ARTIFACTS_GOBY3_JAR}/goby.jar:${GOBY_DIR}:${RESOURCES_GOBYWEB_SERVER_SIDE_COMMONS_IO_JAR}:${RESOURCES_GOBYWEB_SERVER_SIDE_ICB_GROOVY_SUPPORT_JAR}:${RESOURCES_GOBYWEB_SERVER_SIDE_JSAP_JAR} \
            ${RESOURCES_PROCESS_READS_PROCESS_SAMPLES} \
            --jvm-flags "${JAVA_OPTS}" \
            --goby-jar-dir ${RESOURCES_ARTIFACTS_GOBY3_JAR} \
    ...
    if [ ! $RETURN_STATUS -eq 0 ]; then
     ...
     REGISTERED_TAGS=`${FILESET_COMMAND} --push $ATTRIBUTES_TO_ATTACH -a QUALITY_TAGS="${QUALITY_REGISTERED_TAGS}" -a STATS_TAGS="${OUTPUT_STATS_REGISTERED_TAGS}" COMPACT_READ_FILES: *.compact-reads`
