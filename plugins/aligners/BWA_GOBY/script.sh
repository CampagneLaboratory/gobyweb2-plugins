...
function plugin_align {
    ...
    SAMPE_SAMSE_OPTIONS="${PLUGINS_ALIGNER_BWA_GOBY_SAMPE_SAMSE_OPTIONS}"
    ...
    if [ "${PAIRED_END_ALIGNMENT}" == "true" ]; then
        ...
        nice ${BWA_GOBY_EXEC_PATH} aln -w 0 -t ${BWA_GOBY_NUM_THREADS} ${COLOR_SPACE_OPTION} -f ${SAI_FILE_0} -l ${INPUT_READ_LENGTH} ${ALL_OTHER_OPTIONS} -x ${START_POSITION} -y ${END_POSITION} ${INDEX_DIRECTORY}/${INDEX_PREFIX} ${READS_FILE}
        RETURN_STATUS=$?
        if [ $RETURN_STATUS -eq 0 ]; then
            nice ${BWA_GOBY_EXEC_PATH} aln -w 1 -t ${BWA_GOBY_NUM_THREADS} ${COLOR_SPACE_OPTION} -f ${SAI_FILE_1} -l ${INPUT_READ_LENGTH} ${ALL_OTHER_OPTIONS} -x ${START_POSITION} -y ${END_POSITION} ${INDEX_DIRECTORY}/${INDEX_PREFIX} ${READS_FILE}
            RETURN_STATUS=$?
            if [ $RETURN_STATUS -eq 0 ]; then
                # aln worked, let's sampe
                nice ${BWA_GOBY_EXEC_PATH} sampe ${COLOR_SPACE_OPTION} -F goby -f ${OUTPUT}  ${SAMPE_SAMSE_OPTIONS}  -x ${START_POSITION} -y ${END_POSITION} ${INDEX_DIRECTORY}/${INDEX_PREFIX} ${SAI_FILE_0} ${SAI_FILE_1} ${READS_FILE} ${READS_FILE} -r ${READ_GROUPS}
                RETURN_STATUS=$?
            fi
        fi
    else
        ...
        if [ $RETURN_STATUS -eq 0 ]; then
            ...
            nice ${BWA_GOBY_EXEC_PATH} samse ${COLOR_SPACE_OPTION} -F goby -f ${OUTPUT}  ${SAMPE_SAMSE_OPTIONS}  -x ${START_POSITION} -y ${END_POSITION} ${INDEX_DIRECTORY}/${INDEX_PREFIX} ${SAI_FILE_0} ${READS_FILE} -r ${READ_GROUPS}
            RETURN_STATUS=$?
        fi
    fi
}
